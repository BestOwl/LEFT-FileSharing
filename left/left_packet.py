...
class LeftPacket:
    ...
    def to_bytes(self):
        ...
        total_len = 1 + sz_packet_len + sz_target + sz_version
        return pack(f"c{sz_packet_len}s{sz_target}s{sz_version}s",
                    self.opcode, pack("!H", total_len), b_target, b_version)
...
def read_packet_from_socket(socket_read: socket) -> LeftPacket:
    ...
    while unread_packet_len > 0:
        ...
        if header_id == HEADER_F1_VERSION:
            ...
            unread_packet_len -= 4
        elif header_id == HEADER_END_OF_HEADERS:
